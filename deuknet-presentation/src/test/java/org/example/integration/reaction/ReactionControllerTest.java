package org.example.integration.reaction;
...
class ReactionControllerTest extends AbstractTest {
    ...
    void removeReaction() throws Exception {
        UUID postId = createPost();
        UUID reactionId = addReaction(postId);

        mockMvc.perform(delete("/api/posts/" + postId + "/reactions/" + reactionId))
                .andExpect(status().isNoContent());
    }

    private UUID addReaction(UUID postId) throws Exception {
        AddReactionRequest req = new AddReactionRequest();
        req.setReactionType("LIKE");

        MvcResult r = mockMvc.perform(post("/api/posts/" + postId + "/reactions")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(req)))
                .andExpect(status().isCreated())
                .andReturn();
