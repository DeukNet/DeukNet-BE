...
public class OutboxDataChangeEventPublisher implements DataChangeEventPublisher {
     ...
     * @param payload 발행할 객체 (자동으로 JSON 직렬화)
     */
    @Override
    @Transactional
    public void publish(String eventType, UUID aggregateId, Object payload) {
        try {
            ...
            // 3. Outbox 테이블에 저장 (트랜잭션과 함께 커밋됨)
            outboxEventRepository.save(outboxEvent);

            log.debug("Event saved to outbox: type={}, aggregateId={}", eventType, aggregateId);

        } catch (JsonProcessingException e) {
            log.error("Failed to serialize event payload: type={}, aggregateId={}", eventType, aggregateId, e);
