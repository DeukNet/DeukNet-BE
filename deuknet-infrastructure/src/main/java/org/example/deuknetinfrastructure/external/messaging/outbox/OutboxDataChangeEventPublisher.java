...
public class OutboxDataChangeEventPublisher implements DataChangeEventPublisher {
     ...
     *
     * @param eventType 이벤트 타입 (EventType enum)
     * @param aggregateId Aggregate ID
     * @param projection 발행할 Projection 객체 (자동으로 JSON 직렬화), null 가능 (삭제 이벤트 등)
     */
    @Override
    @Transactional
    public void publish(EventType eventType, UUID aggregateId, Projection projection) {
        try {
            ...
            OutboxEvent outboxEvent = new OutboxEvent(
                ...
                eventType.getTypeName(),             // type (EventType enum에서 문자열 추출)
                jsonPayload,                         // payload
                System.currentTimeMillis()           // timestamp (epoch millis)
            );

            // 4. Outbox 테이블에 저장 (트랜잭션과 함께 커밋됨)
            // Debezium이 CDC를 통해 자동으로 감지하여 메시지로 발행
            outboxEventRepository.save(outboxEvent);

            log.debug("Event saved to outbox: aggregateType={}, eventType={}, aggregateId={}",
                aggregateType, eventType.getTypeName(), aggregateId);

        } catch (JsonProcessingException e) {
            log.error("Failed to serialize projection: type={}, aggregateId={}", eventType.getTypeName(), aggregateId, e);
